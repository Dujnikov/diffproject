...
#Check OS distrib-type deb rh ...
LIBERICA_VER=$(git ls-remote --tags git://github.com/bell-sw/Liberica.git | grep 8u | grep -o '8u[[:alnum:]]*[[:punct:]][[:alnum:]]'| sort -r | head -n 1)
echo 'Actual Liberica JDK8 version is '$LIBERICA_VER

echo 'Remove old version Liberica JDK'
if [ -d /opt/bellsoft*.* ]; then rm -f /opt/bellsoft*.*
fi

wget -P /opt https://download.bell-sw.com/java/${LIBERICA_VER}/bellsoft-jdk${LIBERICA_VER}-linux-amd64.deb
wget -p /opt https://download.bell-sw.com/java/${LIBERICA_VER}/bellsoft-jdk${LIBERICA_VER}-linux-amd64.rpm
...
if [ "$OS_VERSION" == "rhel" ]; then
        yum install -y /opt/bellsoft-jdk8*-linux-amd64.rpm
        else
          if [ "$OS_VERSION" == "debian" ]; then
             dpkg -i /opt/bellsoft-jdk8*-linux-amd64.deb
          fi
fi
...
#setup database
echo "=================================================="
echo "This will DESTROY your database ${aeca_database_host}:${aeca_database_port}/${aeca_database_name} if it exists"
echo "Do you want to do this?"
echo "Type [Yes] if want to drop any existing tables and create new"
echo "Type [No] if you already have configured database and want to save current data"
echo "Type [Cancel] to cancel this installation"
select yn in "Yes" "No" "Cancel"; do
  case $yn in
      Yes )  ejbca_database_setup; break;;
      No ) break;;
      Cancel ) exit;;
  esac
done
...
echo "=================================================="
superadmin_password=$(grep '^admin_password' ${aeca_install_directory}/generated_passwords.txt | awk -F= '{ print $2 }' | grep -v '^$')
