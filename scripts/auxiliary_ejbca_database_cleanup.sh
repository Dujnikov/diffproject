...
drop_tables_ejbca() {
    if [ ${aeca_database_type} = "mariadb" ]; then
        script_name="drop-tables-ejbca-mysql.sql"
        if [ -f "$ABSDIR/../dist/sql/${script_name}" ]; then
            echo 'Your script '$${script_name} && cat "$ABSDIR/../dist/sql/${script_name}" | mysql --host=${aeca_database_host} --user=${aeca_database_username} --password=${aeca_database_password} ${aeca_database_name} -f > /dev/null
        else
        ...
        script_name="drop-tables-ejbca-postgres.sql"
        if [ -f "$ABSDIR/../dist/sql/${script_name}" ]; then
            cat "$ABSDIR/../dist/sql/${script_name}" | psql "postgresql://${aeca_database_username}:${aeca_database_password}@${aeca_database_host}/${aeca_database_name}" > /dev/null  
        else
            echo "[ERROR] Can not find $ABSDIR/../dist/sql/${script_name} !"
            exit 1
        fi
